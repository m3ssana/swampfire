...
Music is generated by a human using **Suno AI** and curated to match each zone's identity and storm phase intensity. Every track must loop cleanly. Zone tracks play as the base layer; storm phase tracks crossfade on top as an intensity overlay. The player should never hear a hard cut — only a creeping sense that things are getting worse.

#### 8.1.1 Music Architecture

```
┌─────────────────────────────────────────────────────┐
│  ZONE BASE LAYER (always playing, per-zone)         │
│  ├── Zone 0: Cypress Creek                          │
│  ├── Zone 1: US-41 Corridor                         │
│  ├── Zone 2: Collier Commons                        │
│  ├── Zone 3: Conner Preserve                        │
│  └── Zone 4: LOLHS / SR-54                          │
├─────────────────────────────────────────────────────┤
│  STORM INTENSITY LAYER (crossfades per phase)       │
│  ├── Phase 1: Tension Bed                           │
│  ├── Phase 2: Rising Urgency                        │
│  ├── Phase 3: Full Pressure                         │
│  └── Phase 4: Panic                                 │
├─────────────────────────────────────────────────────┤
│  ONE-SHOT STINGS (fire-and-forget)                  │
│  ├── Menu Theme (splash screen, ~60s loop)          │
│  ├── Installation Sting (~3s)                       │
│  ├── Launch / Victory (~15s)                        │
│  └── Failure / Death (~8s)                          │
└─────────────────────────────────────────────────────┘
```
...
**Crossfade rules:**
- Zone transitions: 500ms crossfade between zone base tracks
- Storm phase transitions: 2000ms crossfade between intensity layers
- Zone base volume: 0.35 (Phase 1-2), 0.25 (Phase 3-4) — duck under storm intensity
- Storm layer volume: 0.0 (Phase 1), 0.15 (Phase 2), 0.30 (Phase 3), 0.45 (Phase 4)

#### 8.1.2 Zone Base Tracks — Suno Generation Prompts

Each prompt below should be fed into Suno. Request **120-second loops** with clean loop points. Generate 3-4 variants per zone and pick the best fit. All tracks should be in **compatible keys** (Am / Dm / Em family) so zone crossfades don't clash harmonically.

**Zone 0 — Cypress Creek Preserve (Home Base)**
File: `assets/music/zone0_cypress.mp3`

> Suno prompt:
> ```
> Southern gothic ambient, fingerpicked acoustic guitar, subtle slide guitar,
> Spanish moss and cypress swamp atmosphere, Florida wetlands at dusk,
> distant frogs and water drips woven into the music, warm but melancholic,
> key of A minor, 72 BPM, lo-fi tape hiss texture, gentle pad underneath,
> feels like sitting on a porch watching storm clouds build on the horizon,
> seamless loop, no vocals, instrumental only
> ```
> Tags: `ambient`, `southern gothic`, `acoustic`, `atmospheric`, `instrumental`

**Zone 1 — US-41 Commercial Corridor**
File: `assets/music/zone1_us41.mp3`

> Suno prompt:
> ```
> Abandoned strip mall at night, lo-fi hip hop beat with muted 808 bass,
> detuned electric piano chords, distant police sirens sampled and pitched down,
> fluorescent light hum texture, empty parking lot energy, Gulf Coast Florida
> after an evacuation order, eerie but rhythmic, something is coming,
> key of D minor, 85 BPM, vinyl crackle, no vocals, instrumental loop,
> urban decay atmosphere, vaporwave meets southern trap
> ```
> Tags: `lo-fi`, `trap`, `atmospheric`, `dark`, `instrumental`

**Zone 2 — Collier Commons (Publix, Library, Rec Center)**
File: `assets/music/zone2_collier.mp3`

> Suno prompt:
> ```
> Suburban neighborhood before a hurricane, nostalgic piano melody over soft
> synth pads, wind chime samples, sprinkler rhythm as percussion, the last
> normal day before everything changes, bittersweet and slightly unsettling,
> like a music box slowing down, key of E minor, 68 BPM, reverb-heavy,
> empty playground feeling, no vocals, instrumental ambient, Midwest emo
> meets ambient electronic
> ```
> Tags: `ambient`, `piano`, `nostalgic`, `melancholic`, `instrumental`

**Zone 3 — Conner Preserve (RC Field, Fire Tower)**
File: `assets/music/zone3_conner.mp3`

> Suno prompt:
> ```
> Deep Florida swamp wilderness, primal and isolated, low drone with
> bottleneck slide guitar, tribal percussion using found objects and
> wood blocks, alligator-in-the-water tension, thick humidity you can
> feel, bayou noir soundtrack, deep sub-bass pulses like a heartbeat,
> key of A minor, 60 BPM, the most dangerous zone, nature is watching,
> no vocals, instrumental dark ambient, Nick Cave meets Ennio Morricone
> in a swamp
> ```
> Tags: `dark ambient`, `cinematic`, `swamp`, `slide guitar`, `instrumental`

**Zone 4 — LOLHS / SR-54 Highway**
File: `assets/music/zone4_lolhs.mp3`

> Suno prompt:
> ```
> Empty high school hallways and exposed highway, industrial ambient,
> metallic percussion, distant thunder rumbles as rhythm, locker door
> slams sampled as snare hits, fluorescent buzz drone, nowhere to hide
> on the open road, tension building with no release, synth bass
> octaves pulsing, key of D minor, 90 BPM, Nine Inch Nails meets
> hurricane preparedness PSA, no vocals, instrumental, driving and
> anxious
> ```
> Tags: `industrial`, `dark electronic`, `cinematic`, `tense`, `instrumental`

#### 8.1.3 Storm Intensity Layers — Suno Generation Prompts

These are **music-only intensity layers** (no SFX — wind/rain/thunder are handled by the ambient system in 8.2). They crossfade on top of zone base tracks. Request **60-second loops** — shorter loops for tighter crossfade control.

**Phase 1 — Distant Warning (60:00–45:00)**
File: `assets/music/storm_phase1.mp3`

> Suno prompt:
> ```
> Barely-there tension bed, single sustained synth pad, very slow LFO
> filter sweep, almost subliminal, like the barometric pressure dropping,
> you feel it more than hear it, sub-bass rumble at the edge of hearing,
> key of A minor, no rhythm, no melody, pure atmosphere, drone ambient,
> volume should sit underneath everything else, 60 second seamless loop,
> no vocals, instrumental only
> ```
> Tags: `drone`, `ambient`, `minimal`, `tension`, `instrumental`

**Phase 2 — Storm Approaching (44:59–30:00)**
File: `assets/music/storm_phase2.mp3`

> Suno prompt:
> ```
> Urgency building, pulsing synth arpeggios at 100 BPM, muted kick drum
> quarter notes, the storm is real and it is coming, news helicopter
> energy, clock is ticking, minor key tension that never resolves,
> sidechained pad breathing effect, key of D minor, no vocals,
> instrumental electronic, like the soundtrack when the weather map
> turns red on the news, Trent Reznor scoring a hurricane documentary
> ```
> Tags: `electronic`, `dark`, `pulsing`, `cinematic`, `instrumental`

**Phase 3 — Full Storm (29:59–15:00)**
File: `assets/music/storm_phase3.mp3`

> Suno prompt:
> ```
> Intense driving pressure, distorted bass hits on every beat at 120 BPM,
> dissonant staccato synth stabs, snare rolls building endlessly,
> the hurricane is here and you are outside in it, relentless forward
> momentum, no breathing room, dark DnB meets film score, chromatic
> chord progression that never feels safe, key of D minor, no vocals,
> instrumental only, Hans Zimmer Dunkirk energy
> ```
> Tags: `drum and bass`, `cinematic`, `intense`, `dark`, `instrumental`

**Phase 4 — Landfall Panic (14:59–0:00)**
File: `assets/music/storm_phase4.mp3`

> Suno prompt:
> ```
> Pure panic, 140 BPM relentless breakbeat, distorted to the edge of
> clipping, no melody just rhythm and dread, industrial kicks layered
> with storm-like white noise sweeps, the eye wall is overhead,
> everything is falling apart, atonal stabs, reverse cymbal swells
> every 4 bars, this is the end if you do not launch NOW,
> key of D minor, no vocals, instrumental, Aphex Twin meets
> emergency broadcast system, absolute maximum intensity
> ```
> Tags: `breakbeat`, `industrial`, `intense`, `chaotic`, `instrumental`

#### 8.1.4 One-Shot Stings — Suno Generation Prompts

These play once and do not loop. Generate as **short tracks** and trim to length.

**Menu Theme (Splash Screen)**
File: `assets/music/menu_theme.mp3`

> Suno prompt:
> ```
> Florida hurricane game main menu, ominous but inviting, acoustic guitar
> picking over a bed of distant thunder and synth pads, southern gothic
> meets survival game, the calm before the storm, makes you want to hit
> start, key of A minor, 75 BPM, 60 seconds, builds slightly but never
> peaks, loop-friendly ending that returns to the opening, no vocals,
> instrumental, like the Red Dead Redemption menu if it was set in
> a Florida swamp
> ```
> Tags: `cinematic`, `acoustic`, `southern gothic`, `atmospheric`, `game soundtrack`

**Installation Sting (Rocket System Installed)**
File: `assets/music/sting_install.mp3`

> Suno prompt:
> ```
> Short triumphant sting, 3 seconds, brass swell with orchestral hit,
> the feeling of snapping a critical piece into place, hope in the
> middle of chaos, major chord resolution after tension, one moment
> of pure "yes" before the storm pulls you back, cinematic,
> no vocals, orchestral + synth hybrid
> ```
> Tags: `cinematic`, `orchestral`, `triumphant`, `short`, `game sfx`

**Launch / Victory**
File: `assets/music/sting_launch.mp3`

> Suno prompt:
> ```
> Rocket launch victory music, 15 seconds, starts with low rumble
> building into full orchestral + synth crescendo, the catharsis of
> escaping a Category 6 hurricane, choir-like synth pads, timpani
> rolls, the moment the rocket clears the storm clouds and you see
> blue sky, pure emotional release, key of A major (shift from minor),
> no vocals, cinematic orchestral, like the ending of Interstellar
> compressed into 15 seconds
> ```
> Tags: `cinematic`, `orchestral`, `triumphant`, `epic`, `game soundtrack`

**Failure / Death**
File: `assets/music/sting_failure.mp3`

> Suno prompt:
> ```
> Game over death sting, 8 seconds, single low cello note sustaining
> then slowly fading to nothing, the hurricane won, all effort wasted,
> gut-punch followed by empty silence, maybe a single distant thunder
> rumble at the tail end, devastating and final, no vocals,
> orchestral dark ambient, the sound of losing everything
> ```
> Tags: `dark`, `orchestral`, `somber`, `cinematic`, `game over`

#### 8.1.5 Music Implementation Notes

- **File format:** MP3 (Suno default), 128kbps minimum. Phaser `sound.add()` handles decode.
- **Loop points:** Trim Suno output in Audacity to create clean loops. Zero-cross at splice points.
- **Total tracks:** 13 (5 zone + 4 storm + 4 stings). Budget ~15MB total compressed audio.
- **Fallback:** If a zone track fails to load, fall back to the menu theme at reduced volume.
- **Mute toggle:** Respect `localStorage.musicMuted` — some players run their own audio.
- **Human workflow:** Generate 3-4 Suno variants per prompt → pick best → trim/loop in Audacity → export MP3 → commit to `public/assets/music/`
