...
Full pattern (confirmed working 2026-03-28) is in `.kiro/steering/agent-instructions.md` → "GitHub Issue Dependencies" section.
...
### Required issue update cadence (MANDATORY — see agent-instructions.md)
...
- `.kiro/skills/` = project-scoped path (committed to repo) — **this is the correct location**
- `~/.kiro/skills/` = user-global — works but not needed; avoid duplicating here
...
- Skills are wired into an agent via `skill://` resources (e.g. `skill://.kiro/skills/**/SKILL.md`); metadata loads at startup, full content loads on demand.
