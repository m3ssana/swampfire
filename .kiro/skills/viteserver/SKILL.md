...
> Note: the listing commands below assume a POSIX shell (Linux/macOS or WSL/Git Bash on Windows). On native Windows PowerShell, use `Get-Process node` / `Get-NetTCPConnection` to inspect running servers instead.
